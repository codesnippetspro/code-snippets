...
<form method="get" action="" id="cloud-search-form">
	...
	<div class="input-group bundle-group">
		...
		<p class="bundle-share-text">
			<?php esc_html_x( 'OR', 'bundle share input', 'code-snippets' ); ?>
		</p>
		...
		<select id="cloud-bundles" class="select-bundle" name="cloud_bundles">
			...
			<?php foreach ( Cloud\Cloud_API::get_bundles() as $bundle ) { ?>
				<option value="<?php echo esc_attr( $bundle['id'] ); ?>"<?php selected( $bundle['id'], $bundle_id ); ?>>
					<?php echo esc_html( $bundle['name'] ); ?>
				</option>
			<?php } ?>
		</select>

		<button type="submit" id="cloud-bundle-show" class="button" name="cloud-bundle-show" value="true">
			<?php esc_html_e( 'Show', 'code-snippets' ); ?>
		</button>

		<button type="submit" id="cloud-bundle-run" class="button" name="cloud-bundle-run" value="true">
			<?php esc_html_e( 'Get Snippets', 'code-snippets' ); ?>
		</button>
	</div>
</form>
