...
test.describe('Contextual Help', () => {
	for (const screen of SCREENS_WITH_HELP) {
		test(`${screen.name} screen exposes its Help tabs`, async ({ page }) => {
		...
		test(`${screen.name} screen opens the first Help topic`, async ({ page }) => {
			await page.goto(screen.url)

			await page.locator('#contextual-help-link').click()
			const help = page.locator('#contextual-help-wrap')
			const firstTopic = help.locator('.contextual-help-tabs a').first()
			const panelId = await firstTopic.getAttribute('href')

			expect(panelId).toMatch(/^#.+/)
			await firstTopic.click()
			await expect(help.locator(panelId ?? '')).toBeVisible()
		})
	}
})
