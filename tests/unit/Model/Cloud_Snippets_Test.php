...
class Cloud_Snippets_Test extends UnitTestCase {
	...
	/**
	 * The page size is read from the meta block alongside the other counts.
	 *
	 * @return void
	 */
	public function test_reads_per_page_from_meta(): void {
		$result = Cloud_Snippets::unpack_api_response(
			[
				'snippets' => [],
				'meta'     => [
	...
	/**
	 * A requested page size wins over the reported one, so callers can compare it
	 * against a later request even when an older cloud omits it from the meta.
	 *
	 * @return void
	 */
	public function test_requested_per_page_overrides_meta(): void {
		$without_meta = Cloud_Snippets::unpack_api_response( [ 'snippets' => [] ], 1, 50 );
		$this->assertSame( 50, $without_meta->per_page );

		$with_meta = Cloud_Snippets::unpack_api_response(
			[
				'snippets' => [],
				'meta'     => [ 'per_page' => 20 ],
			],
		...
		$this->assertSame( 50, $with_meta->per_page );
	}

	/**
	 * Page size defaults to zero when neither the response nor the caller states one.
	 *
	 * @return void
	 */
	public function test_per_page_defaults_to_zero(): void {
		$result = Cloud_Snippets::unpack_api_response( [ 'snippets' => [] ] );

		$this->assertSame( 0, $result->per_page );
	}
}
